...
def create_guidance_system_message() -> str:
    ...
    return (
        "You are an experienced IT service desk agent. Use only the current "
        ...
        "actionable next steps for the assigned agent. Do not invent details; "
        "cite the supporting evidence when you recommend an action or decision."
    )
...
def create_ai_guidance_prompt_with_sources(
    *,
    ...
    guidance_instructions = dedent(
        ...
        If multiple solutions are plausible, explain the variance and prefer
        the most likely outcome based on the strongest evidence. When key
        details are missing, include clarifying questions in
        ...
        to the closest historical matches and call out any mismatches that may
